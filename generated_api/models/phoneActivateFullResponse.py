...
from . import ActivatePhoneProcessTypeId, IsFalloutType, IsCanBeReactivatedType, BaseCfsActionSpecIdTypeId, PaSubscriptionService, PaSwwCatsAddCfs, PaOrderAdd_fs
...
class PhoneActivateFullResponse(BaseModel):
    ...
    updateApplicationName: Optional[constr(max_length=255)] = Field(None, title='updateApplicationName', description='Приложение через которое обновляли запрос', examples=['FE-PHONE'])
    ...
    paOrderAddCfsItems: Optional[List[PaOrderAdd_fs]] = Field(None, title='Paorderaddcfsitems', description='Массив команд для оборудования', min_items=1, items={'$ref': '#/components/schemas/paOrderAdd_fs'})
    ...
    refTownStateId: Optional[int] = Field(None, title='refTownStateId', description='Id Населенного пункта', examples=[5])

    refTownStateName: Optional[constr(max_length=255)] = Field(None, title='refTownStateName', description='Нас. пункт наименование', examples=[' г.Шымкент '])

    exchangeId: Optional[int] = Field(None, title='exchangeId', description='Ид ЦАТС/SWW', examples=[5])

    exchangeName: Optional[constr(max_length=255)] = Field(None, title='exchangeName', description='Наименование станции', examples=['Astana_GTS_SSW_CS2000'])

    refVendorModelId: Optional[int] = Field(None, title='refVendorModelId', description='Ид модели ЦАТС/SWW', examples=[100])

    refVendorModelName: Optional[constr(max_length=255)] = Field(None, title='refVendorModelName', description='Наименование модели ЦАТС/SWW', examples=['Genband CS2000C'])
    ...
    feRejectReasonId: Optional[int] = Field(None, title='feRejectReasonId', description='Идентификатор причины отказа', examples=[0])
    ...
    isCanBeReactivated: Optional[IsCanBeReactivatedType] = Field(None, title='isCanBeReactivated', description='Если 1, дает возможность повторной активации', examples=[0, 1])
